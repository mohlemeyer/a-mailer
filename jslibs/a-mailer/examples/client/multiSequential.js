...
/**
 * Simple example of sending multiple emails sequentially over the same
 * tcp connection.
 * 
...
smtpClient.on('message', function () {
    ...
    smtpClient.write('Subject: Mail No. ' + (msgsSent + 1) + '\n');
    smtpClient.write('\r\n');
    smtpClient.end('This is the message content from mail no. ' + (msgsSent + 1) + '.');
});

smtpClient.on('ready', function(success, response) {
    if (++msgsSent === MSGS_TO_SEND) {
        smtpClient.removeAllListeners('idle');
        smtpClient.quit();
    }
});
